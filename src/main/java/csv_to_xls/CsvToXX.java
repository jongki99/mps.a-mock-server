...
public class CsvToXX {
    ...
    public static void main(String[] args) {
        ...
        folderPath = "/Users/P170355/Downloads/0000-ticket-temp/9541-issueCustom-v2-testXlsx/create-issue-custom"; // 실제 폴더 경로로 변경하세요
    ...
    private static void convertCsvToXlsx(File csvFile) throws IOException {
        try (
            FileInputStream fis = new FileInputStream(csvFile);
            CSVParser parser = new CSVParser(new InputStreamReader(fis, StandardCharsets.UTF_8), CSVFormat.DEFAULT);
            Workbook workbook = new XSSFWorkbook();  // HSSFWorkbook을 사용하여 XLS 파일 생성
            FileOutputStream outputStream = new FileOutputStream(getXlsxFilePath(csvFile))
        ) {
            Sheet sheet = workbook.createSheet("Sheet1");
            // 헤더 생성 -- 해더도 똑같던가? 까먹었네.
            int rowNum = 0;
    ...
    private static String getXlsxFilePath(File csvFile) {
        String filePath = csvFile.getAbsolutePath();
        return filePath.substring(0, filePath.lastIndexOf('.')) + ".xlsx";
    }
