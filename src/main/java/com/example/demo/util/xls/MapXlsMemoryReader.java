...
//			log.debug("{}", getRowDataMap());
	...
	public void parseSheet(Sheet worksheet) {
		int lastRowIndex = worksheet.getLastRowNum(); // 해당시트의 row index : 공백 row 가 보통은 없어서...(0-based)
		for(int rowIndex = 0; rowIndex <= lastRowIndex; rowIndex++) { // row=0은 데이터 : 해더없음 
			Row row = worksheet.getRow(rowIndex);
			if(row != null) {
				startRow(rowIndex);
				int lastCellIndex = row.getLastCellNum() - 1; // 한 row당 cell index(cell 개수-1)(1-based)
				
				for (int cellIndex = 0; cellIndex <= lastCellIndex; cellIndex++) {
					Cell cell = row.getCell(cellIndex); // 셀에 담겨있는 값을 읽는다.
					if(cell != null) {
						String value = "";
